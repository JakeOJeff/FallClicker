    ...
    self.items = {
        {
            ...
            draw = function()
                ...
                local h = wH / 2.5
                local text =
                "Welcome to FallClicker! In this quirky clicker game, your goal isn’t just to earn coins, it’s to level up entire maps! Tap to collect, upgrade your environment, and watch as your world evolves with every click. Unlock new areas, boost your earnings, and see how far your fall can go!"
        ...
        },
        {
            ...
            draw = function()
                ...
                love.graphics.setColor(0.3, 0.3, 0.3)
                love.graphics.rectangle("line", gui.coinDisplay.x - 5, gui.coinDisplay.y - 5,
                    gui.coinDisplay.img:getWidth() + 10, gui.coinDisplay.img:getHeight() + 10, 10, 10)
        ...
        },
        {
            ...
            draw = function()
                ...
                love.graphics.setLineWidth(5)
                love.graphics.setColor(0.3, 0.3, 0.3)
                love.graphics.rectangle("line", gui.sidebarButton.x - 5, gui.sidebarButton.y - 5,
                    gui.sidebarButton.img:getWidth() + 10, gui.sidebarButton.img:getHeight() + 10, 10, 10)
        ...
        },
        {
            ...
            draw = function()
                ...
                love.graphics.setLineWidth(5)
                love.graphics.setColor(0.3, 0.3, 0.3)
                love.graphics.rectangle("line", gui.sidebar.elements[1].button.x - 5,
                    gui.sidebar.elements[1].button.y - 5, gui.sidebar.elements[1].button.img:getWidth() + 10,
                    gui.sidebar.elements[1].button.img:getHeight() + 10, 10, 10)
        ...
        },
        {
            ...
            draw = function()
                ...
                love.graphics.setLineWidth(5)
                love.graphics.setColor(0.3, 0.3, 0.3)
                love.graphics.rectangle("line", gui.sidebar.elements[2].button.x - 5,
                    gui.sidebar.elements[2].button.y - 5, gui.sidebar.elements[2].button.img:getWidth() + 10,
                    gui.sidebar.elements[2].button.img:getHeight() + 10, 10, 10)
...
function tutorial:update(dt)
    ...
    for i, v in ipairs(self.items) do
        if not v.done and type(v.condition) == "function" and v.condition() then
            v.done = true
            if v.postFunc then v.postFunc() end
            break
        end
    end
end
...
function tutorial:checkClick()
    ...
    if self.cooldown <= 0 then
        if isDown and not self.wasMouseDown then
...
function tutorial:mousepressed(x, y, button)
    if button ~= 1 or self.cooldown > 0 then return end

    for i, v in ipairs(self.items) do
        if not v.done then
            if v.condition == "click" then
                v.done = true
                if v.postFunc then v.postFunc() end
                self.cooldown = self.cooldownTime
            end
